...
void main()
{
	...
	vec3 reflected = sampleSoftSkybox(reflectDir, 3.2);
	vec3 refracted = sampleSoftSkybox(refractDir, 4.6);
	...
	float envAmount = mix(0.06, 0.50, rim);
	vec3 color = mix(refractedScene, envColor * vec3(0.42, 0.78, 0.92), envAmount);
	...
	color += vec3(0.12, 0.48, 0.58) * softRim * 0.58;
	color += vec3(0.10, 0.28, 0.32) * shellGlow * 0.18;
	color += vec3(0.78, 1.0, 1.0) * glint * 0.42;
